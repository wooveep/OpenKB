...
DOCUMENT_VERSION_CATALOG_MIGRATION_STATEMENTS: tuple[str, ...] = (
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS knowledge_generation_dossier_sections (
        generation_id INTEGER NOT NULL,
        identity_id TEXT NOT NULL,
        section_ordinal INTEGER NOT NULL CHECK(section_ordinal >= 0),
        title TEXT NOT NULL,
        purpose TEXT NOT NULL CHECK(purpose IN (
            'identity_and_role', 'composition', 'capabilities', 'applicability',
            'requirements', 'operations', 'limitations', 'troubleshooting',
            'version_evolution', 'related_identities', 'details'
        )),
        PRIMARY KEY(generation_id, identity_id, section_ordinal),
        FOREIGN KEY(generation_id, identity_id)
            REFERENCES knowledge_generation_dossier_plans(generation_id, identity_id)
            ON DELETE CASCADE
    )
    """,
    """
    CREATE TABLE IF NOT EXISTS knowledge_generation_dossier_units (
        generation_id INTEGER NOT NULL,
        identity_id TEXT NOT NULL,
        section_ordinal INTEGER NOT NULL,
        unit_ordinal INTEGER NOT NULL CHECK(unit_ordinal >= 0),
        presentation TEXT NOT NULL CHECK(presentation IN ('paragraph', 'list', 'table')),
        claim_ids_json TEXT NOT NULL,
        PRIMARY KEY(generation_id, identity_id, section_ordinal, unit_ordinal),
        FOREIGN KEY(generation_id, identity_id, section_ordinal)
            REFERENCES knowledge_generation_dossier_sections(
                generation_id, identity_id, section_ordinal
            ) ON DELETE CASCADE
    )
    """,
    """
    CREATE TABLE IF NOT EXISTS knowledge_generation_dossier_claim_placements (
        generation_id INTEGER NOT NULL,
        identity_id TEXT NOT NULL,
        ...
        placement_kind TEXT NOT NULL CHECK(placement_kind IN ('summary', 'unit')),
        section_ordinal INTEGER,
        unit_ordinal INTEGER,
        display_ordinal INTEGER NOT NULL CHECK(display_ordinal >= 0),
        PRIMARY KEY(generation_id, identity_id, claim_id),
        FOREIGN KEY(generation_id, identity_id)
            REFERENCES knowledge_generation_dossier_plans(generation_id, identity_id)
            ON DELETE CASCADE,
        ...
        CHECK(
            (placement_kind = 'summary' AND section_ordinal IS NULL AND unit_ordinal IS NULL)
            OR
            (placement_kind = 'unit' AND section_ordinal IS NOT NULL AND unit_ordinal IS NOT NULL)
        )
    )
    """,
    """
    CREATE INDEX IF NOT EXISTS knowledge_generation_dossier_claim_origin_idx
        ON knowledge_generation_dossier_claim_placements(
            candidate_generation_id, candidate_id, claim_ordinal
        )
    """,
    """
