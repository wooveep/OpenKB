...
from openkb.importing.clock import lease_expiry, timestamp
from openkb.importing.reparse import reset_parser_checkpoints_in
from openkb.importing.store import IMPORT_STAGES, ImportJobState
from openkb.importing.types import (
    DesktopRecoveryOverride,
    DesktopStageRun,
)
from openkb.locks import kb_ingest_lock, kb_read_lock
from openkb.parsers.runtime import require_parser_mode
...
class DesktopImportRecoveryStore:
    ...
    def begin(self, job_id: str, override: DesktopRecoveryOverride) -> ImportJobState:
        ...
        with kb_ingest_lock(self._state_dir):
            ...
            try:
                ...
                if override.parser_mode is not None:
                    reset_parser_checkpoints_in(connection, job_id, stage_ids, now)
                ...
                connection.execute(
                    ...
                    INSERT INTO recovery_runs (
                        ...
                        initial_timeout_seconds, status, started_at, completed_at, parser_mode
                    ) VALUES (?, ?, ?, ?, ?, 'running', ?, NULL, ?)
                    """,
                    (
                        recovery_run_id,
    ...
    def parser_mode(self, job_id: str, default: str) -> str:
        """Retain the job's explicit choice across pauses and process restarts."""
        with kb_read_lock(self._state_dir):
            connection = _connect(self._database_path)
            try:
                row = connection.execute(
                    "SELECT parser_mode FROM recovery_runs WHERE job_id = ? "
                    "AND parser_mode IS NOT NULL ORDER BY rowid DESC LIMIT 1",
                    (job_id,),
                ).fetchone()
            finally:
                connection.close()
