...
from openkb.knowledge.pages.adoption import DesktopKnowledgeAdoptionService
from openkb.knowledge.pages.service import DesktopKnowledgePageService
from openkb.knowledge.workspace import DesktopKnowledgeWorkspaceService
...
def dispatch_knowledge_page_request(
    ...
    with server._workspace_requests_lock:
        ...
        if request.method == "workbench.knowledge_workspace_item":
            authority = required_string_param(request, "authority")
            try:
                if authority == "generated":
                    return workspace_service.generated_item(
                        _required_integer_param(request, "generation_id"),
                        required_string_param(request, "item_key"),
                    )
                if authority == "user":
                    return workspace_service.user_item(required_string_param(request, "page_id"))
            except ValueError as error:
        ...
        if request.method == "workbench.knowledge_page":
            return service.select_page(required_string_param(request, "page_id")).as_dict()
        if request.method == "workbench.search_knowledge_sources":
            return {
                "sources": [
                    source.as_dict()
                    for source in service.search_sources(required_string_param(request, "query"))
                ]
            }
            ...
            try:
                return (
                    ...
                    .adopt(
                        ...
                        item_key=required_string_param(request, "item_key"),
                        request_id=required_string_param(request, "adoption_request_id"),
        ...
        if request.method == "workbench.export_knowledge_bundle":
            return (
                DesktopKnowledgeExportService(Path(active.kb_dir))
                .export(
                    Path(required_string_param(request, "destination")),
        ...
        if request.method == "workbench.publish_knowledge_page":
            return service.publish(required_string_param(request, "page_id")).as_dict()
        if request.method == "workbench.verify_knowledge_page":
            return service.verify(required_string_param(request, "page_id")).as_dict()
        if request.method == "workbench.set_knowledge_page_stale_after":
            return service.set_stale_after(
                required_string_param(request, "page_id"),
                _optional_stale_after_param(request),
            ).as_dict()
        if request.method == "workbench.deprecate_knowledge_page":
            return service.deprecate(required_string_param(request, "page_id")).as_dict()
        if request.method == "workbench.restore_knowledge_page":
            return service.restore(required_string_param(request, "page_id")).as_dict()
        if request.method == "workbench.permanently_delete_knowledge_page":
            page_id = required_string_param(request, "page_id")
            service.permanent_delete(
                page_id,
                confirmation_page_id=required_string_param(request, "confirmation_page_id"),
            )
            return {"page_id": page_id, "deleted": True}
        if request.method == "workbench.bind_knowledge_page_source":
            return service.bind_source(
                required_string_param(request, "page_id"),
                required_string_param(request, "claim_text"),
                required_string_param(request, "evidence_id"),
            ).as_dict()
        return service.save_draft(
            page_id=_optional_page_id(request),
            kind=required_string_param(request, "kind"),
            title=required_string_param(request, "title"),
