...
from openkb.engine.version_scope import optional_version_filter_param
...
def dispatch_conversation_request(
    ...
    conversation_id = required_string_param(request, "conversation_id")
    ...
    if request.method == "workbench.rename_conversation":
        return service.rename(conversation_id, required_string_param(request, "title"))
    ...
    if request.method == "workbench.select_answer_version":
        return service.select_answer_version(
            conversation_id,
            required_string_param(request, "assistant_message_id"),
            required_string_param(request, "answer_version_id"),
        )
    ...
    if request.method == "workbench.ask_conversation":
        return service.ask(
            conversation_id,
            required_string_param(request, "question"),
            on_delta=on_delta,
