...
from openkb.documents.version_labels import VersionScheme
    ...
    with server._workspace_requests_lock:
        ...
        if request.method == "workbench.document_version_diffs":
            return {
                "diffs": [
                    asdict(diff)
                    for diff in service.list_diffs(required_string_param(request, "lineage_id"))
                ]
            }
        ...
        decision = required_string_param(request, "decision")
        ...
        return service.resolve_candidate(
            required_string_param(request, "candidate_id"), decision
        ).as_dict()
