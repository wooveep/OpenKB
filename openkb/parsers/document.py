...
from openkb.documents.usability import assess_document_ir
...
def parse_structured_document(
    ...
    *,
    ...
    if source_format in {"doc", "ppt"}:
        from openkb.parsers.legacy_office import parse_legacy_office_document

        return parse_legacy_office_document(source, raw_bytes, source_format)
    if source_format in {"xls", "xlsx"}:
        from openkb.parsers.spreadsheet import parse_spreadsheet_document

        return parse_spreadsheet_document(source, raw_bytes, source_format)
    if source_format == "pptx":
        from openkb.parsers.presentation import parse_presentation_document
    ...
    if source_format == "pdf":
        from openkb.parsers.pdf import parse_pdf_document
...
def _maybe_enhance_office(
    ...
    *,
    ...
    if parser_mode == "auto" and report.usable:
        return parsed
    from openkb.parsers.office_ocr import enhance_office_document
