...
def request_cancel_in(
    ...
    row = connection.execute(
        "SELECT status, retry_scope FROM document_page_tree_enrichment_tasks "
        "WHERE document_id = ?",
        (document_id,),
    ).fetchone()
...
def retry_document_in(
    ...
    *,
    ...
    previous_scope = (
        str(previous[0]) if previous is not None and previous[0] is not None else None
    )
    cursor = connection.execute(
        """
        UPDATE document_page_tree_enrichment_tasks
