...
def reset_parser_runtime_for_tests() -> None:
    ...
    with _entries_lock:
        for family in _entries:
            _entries[family] = _RuntimeEntry(
                "ready" if family == "native_office" else "not_loaded"
            )
...
def _initialize_family(family: ParserFamily, source: Path | None) -> None:
    if family == "legacy_office":
        from openkb.desktop_legacy_office_parsers import prewarm_legacy_office_runtime

        prewarm_legacy_office_runtime(source)
        return
    if family == "pdf_ocr":
        from openkb.desktop_pdf_parsers import warm_pdf_ocr_runtime
...
def _legacy_resources_available() -> bool:
    try:
        from openkb.desktop_legacy_office_parsers import legacy_office_resources_available
