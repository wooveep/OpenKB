...
- 状态：历史实施设计；当前语义权威以 ADR 0083 及 ADR 0082 的修订为准
- 日期：2026-09-04
...
当前适用范围：不可变 Candidate Registry、先候选后关系、证据与代次绑定继续有效。
本文中的固定 subtype/关系本体、词表准入规则、旧图谱兼容读取和迁移条款已被
[ADR 0083](../adr/0083-derive-domain-semantics-under-code-owned-evidence-constraints.md)
取代。当前版本拒绝旧 semantic epoch，不提供隐式旧图谱回退。保留下文用于解释设计历史。
