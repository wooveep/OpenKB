...
def _parse_case(value: object, index: int) -> EvaluationCase:
    ...
    evidence = tuple(
        ...
        for item in [_mapping(raw_item, "case.evidence")]
    )
    ...
    for raw_claim in claims_value:
        claim = _mapping(raw_claim, "page.claim")
        ...
        applicability = tuple(
            (
                _text(_mapping(item, "claim.applicability").get("dimension"), "dimension"),
                _text(_mapping(item, "claim.applicability").get("value"), "value"),
            )
...
def _dimension_ids(value: object, field: str) -> tuple[str, ...]:
    ...
    result = tuple(_text(_mapping(item, field).get("id"), f"{field}.id") for item in value)
